...
import { range } from '@zeitgeistpm/utility/dist/array'
...
export const rpcPrices = async (ctx: RpcContext, query: PricesQuery) => {
  ...
  const prices: [ZeitgeistPrimitivesAsset, Vec<u128>][] = await Promise.all(
    assets.map(async asset => [asset, await ctx.api.rpc.swaps.getSpotPrices(query.pool, ztg, asset, blocks)]),
  )
