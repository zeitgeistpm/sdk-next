...
export const mainnet = async (): Promise<FullContext> => {
...
export const mainnetApi = async (): Promise<ApiContext> => {
  const provider = new WsProvider([
    rpcs.dwellir.uri,
    rpcs.zeitgeist.uri,
    rpcs.onfinality.uri,
...
export const mainnetIndexer = async (): Promise<IndexerContext> => {
  const indexer = Indexer.create({ uri: indexers.zeitgeist.uri })
  return {
    indexer,
  }
...
export const batteryStation = async (): Promise<FullContext> => {
...
export const batteryStationApi = async (): Promise<ApiContext> => {
  const provider = new WsProvider([rpcs.bsr.uri])
  const api = await ApiPromise.create(options({ provider }))
  return {
    api,
...
export const batteryStationIndexer = async (): Promise<IndexerContext> => {
  const indexer = Indexer.create({ uri: indexers.bsr.uri })
  return {
    indexer,
  }
